...
def preprocess_reports(data_path, start, end, sentence=False, image=False):
    ...
    for idx, row in impressions.iterrows():
        if isinstance(row["report"], float) and math.isnan(row["report"]):
            continue
        sen = re.sub(
            "(?<! )(?=[/,-,:,.,!?()])|(?<=[/,-,:,.,!?()])(?! )", r" ", row["report"]
        ).split()
...
def run_inference(model_path, cuda):
    ...
    os.system(
        f"allennlp predict {model_path} {data_path} \
...
def postprocess_reports(data_source, data_split):
    ...
    for file in data:
        postprocess_individual_report(
            file, final_dict, data_source=data_source, data_split=data_split
        )

    return final_dict


def postprocess_individual_report(
    file, final_dict, data_source=None, data_split="inference"
):
...
def _add_ids_column(csv_path, study_id_csv_path, output_path):
