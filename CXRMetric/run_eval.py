...
def calc_metric(
    gt_csv_o, pred_csv_o, out_csv
):  # TODO: support single metrics at a time
    """Computes four metrics and composite metric scores."""
    os.environ["MKL_THREADING_LAYER"] = "GNU"

    gt = (
        pd.read_csv(gt_csv_o).sort_values(by=[STUDY_ID_COL_NAME]).reset_index(drop=True)
    )
    gt_csv = gt_csv_o + ".sorted"
    gt.to_csv(gt_csv)

    pred = (
        pd.read_csv(pred_csv_o)
        .sort_values(by=[STUDY_ID_COL_NAME])
        .reset_index(drop=True)
    )
    pred_csv = pred_csv_o + ".sorted"
    ...
    os.system(
        f"python CXRMetric/CheXbert/src/encode.py -c {CHEXBERT_PATH} -d {pred_csv} -o {pred_embed_path}"
    )
    os.system(
        f"python CXRMetric/CheXbert/src/encode.py -c {CHEXBERT_PATH} -d {gt_csv} -o {gt_embed_path}"
    )
    ...
    run_radgraph(
        gt_csv, pred_csv, cache_path, RADGRAPH_PATH, entities_path, relations_path
    )
    ...
    scores = composite_metric_model.predict(norm_input_data[:, metric_col_indices])
    # append new column
